...
char view_log(unsigned char key, unsigned char reset_flag)
{
    ...
    {
        rpos = 0;   // Start from latest event
        clear_screen();
    }
   ...
   /* add = rpos * 10 + 5; OLD LOGIC*/
    unsigned char index;
    /* Calculate actual EEPROM index (latest first) */
    index = (pos - rpos + 10) % 10;
    add = index * 10 + 5;
